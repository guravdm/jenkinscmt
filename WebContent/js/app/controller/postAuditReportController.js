...
CMTApp.controller('postAuditReportController', ['$scope','$rootScope','$stateParams', '$state', 'ApiCallFactory','$location', 'DataFactory', 'Storage','toaster','spinnerService','$uibModal', '$filter','$window', '$http', function ($scope,$rootScope,$stateParams, $state,ApiCallFactory, $location, DataFactory, Storage,toaster,spinnerService, $uibModal, $filter,$window, $http) {
	...
	$('.panel-complied').click(function() {
		...
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "Complied";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		// {'chart_name':'overall','status':'Complied','entity':"",'unit':"",'department':""}
		$scope.drilledReport($scope.searchObjPostAudit);
		// alert(complied);
	});

	$('.panel-posing').click(function() {
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "PosingRisk";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	...
	$('.panel-noncomplied').click(function() {
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "NonComplied";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	...
	$('.panel-wfapp').click(function() {
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "Waiting For Approval";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	...
	$('.panel-reopened').click(function() {
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "Re-Opened";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	...
	$('.panel-delayed').click(function() {
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "Delayed";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	...
	$('.panel-delayedReported').click(function() {
		console.log('searchObjPostAudit.status : ' + $scope.searchObjPostAudit.status);
		$scope.searchObjPostAudit.chart_name = "overall";
		$scope.searchObjPostAudit.status = "DelayedReported";
		$scope.searchObjPostAudit.entity = 0;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	...
	$scope.entityOneClickEvent = function(ent) {
		...
		$scope.searchObjPostAudit.chart_name = "entityLevelPostAudit";
		$scope.searchObjPostAudit.status = "Complied";
		$scope.searchObjPostAudit.entity = $scope.entityName;
		$scope.searchObjPostAudit.unit = 0;
		$scope.searchObjPostAudit.department = 0;
		$scope.drilledReport($scope.searchObjPostAudit);
	}
	...
	$('#entityLevelPostAudit').bind('dataPointMouseUp', function(e, data) {
...
//		$scope.entityDrilledReport($scope.entityDrilledObj);
		$scope.drilledReport({'chart_name':'entityLevelPostAudit','status':data.series.title,'entity':data.x,'unit':0,'department':0});
	});
	...
	$scope.entityDrilledDown = function(entityName, status) {
		...
		$scope.drilledReport({'chart_name':'entityLevelPostAudit','status':status,'entity':entityName,'unit':0,'department':0});
	}	
...
//	Click Evevent & Dril-Down For Pie Chart
	$('#jqChart1PostAudit').bind('dataPointMouseUp', function (e, data) {
		//console.log('drilledDown report jqChart1PostAudit fun() : ');
		...
		if(data.dataItem[0] === "Complied"){	
			...
			// {'chart_name':'overall','status':'Complied','entity':"",'unit':"",'department':""}
			$scope.drilledReport($scope.searchObjPostAudit);
			// $state.go('DrilledReport',$scope.searchObjPostAudit);
	...
	$('#jqChart2').bind('dataPointMouseUp', function (e, data) {
		if(data.dataItem[0] === "Complied"){		
			$scope.searchObjPostAudit.chart_name = "internal";
			$scope.searchObjPostAudit.status = "Complied";
			$scope.searchObjPostAudit.entity = 0;
			$scope.searchObjPostAudit.unit = 0;
			$scope.searchObjPostAudit.department = 0;
			...
			$scope.searchObjPostAudit.chart_name = "internal";
			$scope.searchObjPostAudit.status = "Delayed-Reported";
			$scope.searchObjPostAudit.entity = 0;
			$scope.searchObjPostAudit.unit = 0;
			$scope.searchObjPostAudit.department = 0;
			$scope.drilledReport($scope.searchObjPostAudit);
...
//	Click Evevent & Dril-Down For Entity Chart
	// $('#entityLevelPostAudit').bind('dataPointMouseUp', function (e, data) {
	//$state.go('DrilledReport',{'chart_name':'entityLevelPostAudit','status':data.series.title,'entity':data.x,'unit':0,'department':0});
//	console.log("e "+e);   
//	console.log("data "+data.x);
...
//					fillStyles: ['#03C523', '#FBFB35', '#F53636', '#C8CFCB','#bfd630','#85c1e9','#f5b041' ],
					fillStyles: ['#03C523 ','#FF0000','#f5b041' ], 
					// , '#f5b041'
					labels: {
						stringFormat: '%.1f%%',
						valueType: 'percentage',
...
//	-- End Graph Code --//
	...
	$scope.drilledReport = function(obj){
		...
		angular.forEach($scope.taskList, function (data) {
			...
			if(obj.chart_name=="entityLevelPostAudit") {
...
//	to generate PDF
	 ...
	 * @author DnyaneshG
	 * @Date 
	 * @return post audit report
	 */
	
	$scope.postAuditReportExl = function() {
		$("#postAuditTableExport tr td a").removeAttr("href");
		$("#postAuditTableExport tr td a").css("color","black");
		$("#postAuditTableExport").table2excel({		
			name: "Report",
			filename: "PostAuditReport_EXL" 
		}); 
	}
	
	
	$scope.postAuditReportPdf = function(){
		var doc1 = new jsPDF('p', 'pt', 'a4');
		var res1 = doc1.autoTableHtmlToJson(document.getElementById("postAuditTableExport"));
		doc1.autoTable(res1.columns, res1.data, {
			startY: 10,
			styles: {
				overflow: 'linebreak',
...
//	}
	...
	$('#countriesGraph').bind('dataPointMouseUp', function (e, data) {
		...
		if(data.dataItem[0] == "Complied"){
			...
			$scope.searchObjPostAudit.chart_name = "entityLevelPieChart";
			$scope.searchObjPostAudit.status = "Complied";
			$scope.searchObjPostAudit.entity = 0;
			$scope.searchObjPostAudit.unit = 0;
			$scope.searchObjPostAudit.department = 0;
			...
			$scope.searchObjPostAudit.unit = 0;
			$scope.searchObjPostAudit.department = 0;
			console.log('$scope.searchObjPostAudit : ' + $scope.searchObjPostAudit);
			$scope.drilledReport($scope.searchObjPostAudit);
		}else if(data.dataItem[0] == "Delayed"){
			$scope.searchObjPostAudit.chart_name = "entityLevelPieChart";
			$scope.searchObjPostAudit.status = "Delayed";
...
//			}, 
	...
	$scope.exportEntityLevelGraphChart = function() {
		...
		$("#entityLevelPostAudit").jqChart('exportToImage', config);
	};
	...
	$scope.exportOverAllGraphChart = function() {
		...
		$("#jqChart1PostAudit").jqChart('exportToImage', config);
	};
...
//	End Multiple task completion code
	...
	$scope.uploadFile=function(formValid) {
		...
		if(!angular.isUndefined($scope.taskObjPostAudit.task_list)) {
			for (var i=0; i<$scope.taskObjPostAudit.task_list.length; i++) {
				formData.append("task_list", $scope.taskObjPostAudit.task_list[i]);
			}
		}
	...
	$scope.monthlyComplianceAuditChart = function() {
		...
		console.log('search monthlyComplianceAuditChart graph() called ');

		spinnerService.show('html5spinner');

		$scope.frmSearchDate = $filter('date')($scope.searchObjPostAudit.date_from, 'yyyy-MM-dd');
		$scope.toSearchDate = $filter('date')($scope.searchObjPostAudit.date_to, 'yyyy-MM-dd');
		console.log('frmSearchDate : ' + $scope.frmSearchDate + '\t toSearchDate : ' + $scope.toSearchDate);
		console.log('frmSearchDate : ' + $scope.frmSearchDate + '\t toSearchDate : ' + $scope.toSearchDate);
		...
		var obj = {
				date_from: $scope.frmSearchDate,
				date_to: $scope.toSearchDate,
				orgaId: $scope.searchObjPostAudit.orga_id,
				locaId: $scope.searchObjPostAudit.loca_id,
				deptId: $scope.searchObjPostAudit.dept_id
		};

		console.log('obj values : ' + JSON.stringify(obj));

		ApiCallFactory.searchComplianceAuditChart(obj).success(function(res, status) {
			spinnerService.hide('html5spinner');
			//$scope.getEntityChart();
			if(status === 200) {
				$("#pieLoader").hide();
