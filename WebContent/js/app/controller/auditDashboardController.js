...
CMTApp.controller('auditDashboardController', ['$scope','$rootScope','$stateParams', '$state', 'ApiCallFactory','$location', 'DataFactory', 'Storage','toaster','spinnerService','$uibModal', '$filter','$window', function ($scope,$rootScope,$stateParams, $state,ApiCallFactory, $location, DataFactory, Storage,toaster,spinnerService, $uibModal, $filter,$window) {
	...
	$scope.dashboardObjAudit.fquarter = 0;
	$scope.entityName = null;
	//nitin
	$scope.AllTasks				= [];
	$scope.searchObjAudit 			= {};
	$scope.configObj = {};
	...
	$scope.dashboardObjAudit.tasks_list = [];
	...
	if(getCurrentMonth >= 4 && getCurrentMonth <= 6){
		$scope.dashboardObjAudit.fmonth = 1;
		console.log('Q1');
	}else if(getCurrentMonth >= 7 && getCurrentMonth <= 9){
		$scope.dashboardObjAudit.fmonth = 2;
		console.log('Q2');
	}else if(getCurrentMonth >= 10 && getCurrentMonth <= 12){
		$scope.dashboardObjAudit.fmonth = 3;
		console.log('Q3');
	}else {
		$scope.dashboardObjAudit.fmonth = 4;
		console.log('Q4');
	}
	...
	$scope.dashboardObjAudit.fyear = curr_year;
	...
	$scope.getAuditorComplianceStatusPieChart = function() {
		...
		$scope.date_from =  new Date(todayMinus.setDate(todayMinus.getDate() - 20));
		$scope.date_to = new Date(todayPlus.setDate(todayPlus.getDate() + 20));
		...
		ApiCallFactory.auditorComplianceStatusPieChart(obj).success(function(res,status){
...
//				$scope.Delayed = res.Delayed;
	...
	$scope.getAuditorComplianceStatusPieChart();
	...
	$('.panel-complied').click(function() {
		...
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "Complied";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		// {'chart_name':'overall','status':'Complied','entity':"",'unit':"",'department':""}
		$scope.drilledReport($scope.searchObjAudit);
		// alert(complied);
	});

	$('.panel-posing').click(function() {
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "PosingRisk";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	...
	$('.panel-noncomplied').click(function() {
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "NonComplied";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	...
	$('.panel-wfapp').click(function() {
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "Waiting For Approval";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	...
	$('.panel-reopened').click(function() {
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "Re-Opened";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	...
	$('.panel-delayed').click(function() {
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "Delayed";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	...
	$('.panel-delayedReported').click(function() {
		console.log('searchObj.status : ' + $scope.searchObjAudit.status);
		$scope.searchObjAudit.chart_name = "overall";
		$scope.searchObjAudit.status = "DelayedReported";
		$scope.searchObjAudit.entity = 0;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	...
	$scope.entityOneClickEvent = function(ent) {
		...
		$scope.searchObjAudit.chart_name = "entityLevel";
		$scope.searchObjAudit.status = "Complied";
		$scope.searchObjAudit.entity = $scope.entityName;
		$scope.searchObjAudit.unit = 0;
		$scope.searchObjAudit.department = 0;
		$scope.drilledReport($scope.searchObjAudit);
	}
...
//	Click Evevent & Dril-Down For Pie Chart
	$('#auditJqChart1').bind('dataPointMouseUp', function (e, data) {
		//console.log('drilledDown report auditJqChart1 fun() : ');
		...
		if(data.dataItem[0] === "Complied"){	
			...
			// {'chart_name':'overall','status':'Complied','entity':"",'unit':"",'department':""}
			$scope.drilledReport($scope.searchObjAudit);
			// $state.go('DrilledReport',$scope.searchObjAudit);
	...
	$('#jqChart2').bind('dataPointMouseUp', function (e, data) {
		if(data.dataItem[0] === "Complied"){		
			$scope.searchObjAudit.chart_name = "internal";
			$scope.searchObjAudit.status = "Complied";
			$scope.searchObjAudit.entity = 0;
			$scope.searchObjAudit.unit = 0;
			$scope.searchObjAudit.department = 0;
			...
			$scope.searchObjAudit.chart_name = "internal";
			$scope.searchObjAudit.status = "Delayed-Reported";
			$scope.searchObjAudit.entity = 0;
			$scope.searchObjAudit.unit = 0;
			$scope.searchObjAudit.department = 0;
			$scope.drilledReport($scope.searchObjAudit);
...
//	console.log("status is "+data.series.title);
	...
	$scope.loadQuarterGraphOnLoad = function() {
		...
		if ($scope.dashboardObjAudit.fquarter == 0) {
...
//		$scope.entityListt  = [];
	...
	$scope.financeMonthGraphLoadFun = function() {
		...
		angular.forEach($scope.quarterToMonthGraph.Month,function(value, key){
			...
			angular.forEach($scope.taskList, function (data) { 
				...
				var date_lag = new Date(legal_Date[2],legal_Date[1]-1,legal_Date[0]);
				// console.log(date_lag);
				// console.log('orga id : ' + $scope.dashboardObjAudit.organisationId);
				if($scope.dashboardObjAudit.organisationId == data.orga_id){
					if((legal_Date[2]>= curr_year  && legal_Date[2] <= next_year) && ((legal_Date[2]== curr_year && date_lag.getMonth() >= 3) || (legal_Date[2] == next_year && date_lag.getMonth() <= 3)) ) {
						if(month[date_lag.getMonth()] === value){
							if(data.status=='complied'){
								fcomplied++;
	...
	$scope.loadOtherGraphs = function() {
		...
		$scope.functionGraph                        = {};
		$scope.functionGraph.OrgaNames  	        = [];
		$scope.functionGraph.UnitsNames    	        = [];
...
//	getEntityUnitFunctionDesignationList
	...
	$scope.getUnits = function(){
		...
		$scope.dept_list = [];
		var orga_id = $scope.dashboardObjAudit.FunlevelEntity;
...
//		$scope.entityListt  = [];
		...
		ApiCallFactory.searchComplianceStatusAuditPieChart(obj).success(function(res,status) {
			spinnerService.show('html5spinner');
			//$scope.getEntityChart();
			if(status === 200) {
				spinnerService.hide('html5spinner');
			...
			series: [
				{
					...
					type: 'pie',
					//fillStyles: ['#8df3bc ', '#fafaa4f0', '#ffa7a7 ', '#e1d5d5', '#dbebf6', '#f3d7a9'],
//					fillStyles: ['#03C523', '#FBFB35', '#F53636', '#C8CFCB','#bfd630','#85c1e9','#f5b041' ],
					fillStyles: ['#03C523 ','#F53636 ','#f5b041' ], 
					// , '#f5b041'
					labels: {
						stringFormat: '%.1f%%',
						valueType: 'percentage',
					...
					explodedSlices: [7],
					data: [
						['Complied', $scope.complianceChartData.Complied], 
			...
			series: [
				{
					...
					//fillStyles: ['#03C523 ', '#fafaa4f0', '#FF0000 ', '#BFBFBF', '#85c1e9', '#f5b041'],
//					fillStyles: ['#03C523 ', '#fafaa4f0', '#FF0000 ', '#BFBFBF','#bfd630','#85c1e9','#f5b041' ], , '#f5b041'
					fillStyles: ['#03C523 ','#F53636 ' ],
					labels: {
						stringFormat: '%.1f%%',
						valueType: 'percentage',
						font: '15px sans-serif',
					...
					explodedSlices: [7],
					data: [
						['Complied', $scope.internalComplianceChartData.Complied], 
...
//		console.log('functionChartData : ' + JSON.stringify($scope.functionChartData));
...
//	Unit Graph -Search entity wise graph
	$scope.searchEntityWise = function(){
		console.log("Entity ID "+$scope.dashboardObjAudit.entity);
		var orga_id = $scope.dashboardObjAudit.entity;
		...
		for(var i = 0; i< loca_list.length; i++){
			...
			angular.forEach($scope.taskList, function (data) { 
				...
				if((!orga_id || data.orga_id === orga_id) && (loca_list[i]===data.loca_id)) {


					if(data.auditorStatus == 'Complied') {
						complied++;
					}
					if(data.auditorStatus == 'NonComplied') {
						noncomplied++;
					}
					if(data.auditorStatus == 'Re_Opened'){
						re_opened++;
					}
				}
...
//				}
	...
	$scope.searchFuntionLevelGraph = function() {
		...
		var orga_id = $scope.dashboardObjAudit.FunlevelEntity;
		var loca_id = $scope.dashboardObjAudit.FunlevelUnit;
		var dept_id = $scope.dashboardObjAudit.FunlevelFunction;

		var loca_list = [];
		var dept_list = [];

		angular.forEach($scope.FunctionList, function (fun_data) {
//			console.log('fun_data : ' + JSON.stringify(fun_data));
		...
		for(var i = 0; i< dept_list.length; i++) {
			...
			angular.forEach($scope.taskList, function (data) { 
				...
				console.log('data auditorStatus : ' + data.auditorStatus);

				var task_date = new Date(data.date);

				if((!orga_id || data.orga_id === orga_id) && (!loca_id || loca_id === data.loca_id) && (dept_list[i]===data.dept_id)) {
					if(data.auditorStatus == 'Complied') {
						complied++;
					}
					if(data.auditorStatus == 'NonComplied') {
...
//				if((!orga_id || data.orga_id === orga_id) && (!loca_id || loca_id === data.loca_id) && (dept_list[i]===data.dept_id) && 
//				(!(from && to) || task_date>=from_date && task_date <=from_to)){
//				if(data.status=='complied'){
//				complied++; 
//				}
...
//			$scope.functionGraph.WaitingForApproval.push(partially_Completed);
			$scope.functionGraph.ReOpened.push(re_opened);
//			$scope.functionGraph.Dealyed_Reported(delayed_reported);
	...
	$scope.drilledReport = function(obj){
		...
		angular.forEach($scope.taskList, function (data) {
			...
			if(obj.chart_name=="departmentLevel"){
				console.log('obj.chart_name == departmentLevel ');
				var orga_id = $scope.dashboardObjAudit.FunlevelEntity;
				var loca_id = $scope.dashboardObjAudit.FunlevelUnit;
			...
			if(obj.chart_name=="financeLevel") {
				var changed_month = $scope.dashboardObjAudit.fmonth;
				var changed_year = $scope.dashboardObjAudit.fyear;
...
//			}, 
	...
	$scope.dashboardObjAudit.orgaId = 'null';
	/**
	 * Hidden
	 */
	...
	$scope.testFun = function() {
		...
		var obj1 = {
				date_from: $scope.frmSearchDate,
				date_to: $scope.toSearchDate,
				orgaId: $scope.dashboardObjAudit.orgaId
		};
		// alert("test fun ");
			...
			if(status === 200){
				...
				angular.forEach($scope.entityQuarterGraph.Quarter, function (value, key) {
					...
					angular.forEach($scope.taskAllList, function (data) {
						...
						if(data.orga_id == $scope.dashboardObjAudit.orgaId) {
							if((legal_Date[2]>= curr_year  && legal_Date[2] <= next_year) && ((legal_Date[2]== curr_year && date_lag.getMonth() >= 3) || (legal_Date[2] == next_year && date_lag.getMonth() <= 3)) ){
...
//	onchange of financial year 
	$scope.getFincialGraphByYear = function(){
		var year = $scope.dashboardObjAudit.fyear;
		//alert(year);
		var orga_list = [];
		var orga_name_list = [];
		var loca_list = [];
		...
		if ($scope.dashboardObjAudit.fmonth == 1) {
			$scope.financialGraph.Month.push("April");
			$scope.financialGraph.Month.push("May");
			$scope.financialGraph.Month.push("June");
		...
		}else if ($scope.dashboardObjAudit.fmonth == 2) {
			$scope.financialGraph.Month.push("July");
			$scope.financialGraph.Month.push("August");
			$scope.financialGraph.Month.push("September");
		...
		else if ($scope.dashboardObjAudit.fmonth == 3) {
			$scope.financialGraph.Month.push("October");
			$scope.financialGraph.Month.push("November");
			$scope.financialGraph.Month.push("December");
		...
		else if ($scope.dashboardObjAudit.fmonth == 4) {
			$scope.financialGraph.Month.push("January");
			$scope.financialGraph.Month.push("February");
			$scope.financialGraph.Month.push("March");
		...
		}else if ($scope.dashboardObjAudit.fmonth == 0) {
...
//			$scope.financialGraph.Delayed_Reported.push(fdelayed_reported);
		...
		if($scope.dashboardObjAudit.fmonth == 0){
			$scope.loadFinanceGraphByQuarter($scope.financialGraph);
		}else{
			$scope.loadFinanceGraph($scope.financialGraph);
		}
...
//			$scope.functionGraph.Delayed_Reported.push(delayed_reported);
	...
	$scope.getFincialGraphByMonth = function(){
		...
		// alert($scope.dashboardObjAudit.fmonth);
		$scope.financialGraph = {};		
		$scope.financialGraph.Month = [];
		$scope.financialGraph.Quarter = [];
		$scope.financialGraph.Complied = [];
		...
		angular.forEach($scope.financialGraph.Month, function (value, key) {
			...
			angular.forEach($scope.taskList, function (data) {
				...
				var date_lag = new Date(legal_Date[2], legal_Date[1] - 1, legal_Date[0]);
				// console.log(date_lag);
				if($scope.dashboardObjAudit.fmonth == 1 || $scope.dashboardObjAudit.fmonth == 2 || $scope.dashboardObjAudit.fmonth == 3)
					if ((legal_Date[2] >= year && legal_Date[2] < next_year) && ((legal_Date[2] == year && date_lag.getMonth() >= 3) || (legal_Date[2] == next_year && date_lag.getMonth() <= 3))) {
						if (month[date_lag.getMonth()] === value) {
							if (data.status == 'complied') {
								fcomplied++;
				...
				if($scope.dashboardObjAudit.fmonth == 4){
					if ((legal_Date[2] >= year && legal_Date[2] <= next_year) && ((legal_Date[2] == year && date_lag.getMonth() >= 3) || (legal_Date[2] == next_year && date_lag.getMonth() <= 3))) {
						if (month[date_lag.getMonth()] === value) {
							if (data.status == 'complied') {
								fcomplied++;
	...
	$scope.getFincialGraphByYear = function(){
		...
		angular.forEach($scope.financialGraph.Month, function (value, key) {
			...
			angular.forEach($scope.taskList, function (data) {
				...
				if ((legal_Date[2] >= year && legal_Date[2] <= next_year) && ((legal_Date[2] == year && date_lag.getMonth() >= 3) || 
						(legal_Date[2] == next_year && date_lag.getMonth() <= 3))) {
					if (month[date_lag.getMonth()] === value) {
						if (data.status == 'complied') {
							fcomplied++;
						}
...
//	}
	...
	$('#countriesGraph').bind('dataPointMouseUp', function (e, data) {
		...
		if(data.dataItem[0] == "Complied"){
			...
			$scope.searchObjAudit.chart_name = "entityLevelPieChart";
			$scope.searchObjAudit.status = "Complied";
			$scope.searchObjAudit.entity = 0;
			$scope.searchObjAudit.unit = 0;
			$scope.searchObjAudit.department = 0;
			...
			$scope.searchObjAudit.unit = 0;
			$scope.searchObjAudit.department = 0;
			console.log('$scope.searchObjAudit : ' + $scope.searchObjAudit);
			$scope.drilledReport($scope.searchObjAudit);
		}else if(data.dataItem[0] == "Delayed"){
			$scope.searchObjAudit.chart_name = "entityLevelPieChart";
			$scope.searchObjAudit.status = "Delayed";
	...
	$scope.loadCountries = function(data) {
		...
		$('#countriesGraph').jqChart({
			...
			series: [
				{
					type: 'pie',
					fillStyles: ['#03C523 ', '#fafaa4f0', '#F53636 ', '#BFBFBF', '#85c1e9', '#f5b041'],
					labels: {
						stringFormat: '%.1f%%',
						valueType: 'percentage',
						font: '15px sans-serif',
...
//						['Waiting For Approval', $scope.myData.WaitingForApproval[0]], 
	...
	$scope.financeLevelGraphFun = function(data) {
		...
		$scope.FinanceChartData = data;
		console.log('financeLevel : ' + $scope.FinanceChartData.Month);
...
//			}, 
	...
	$scope.exportOverAllGraphChart = function() {
		...
		$("#auditJqChart1").jqChart('exportToImage', config);
	};
...
//	Region Level graph jqChart fun
	...
	$scope.searchCategoryWiseGraph = function () {
		...
		var orga_id = $scope.dashboardObjAudit.categoryLevelEntity;
		var loca_id = $scope.dashboardObjAudit.categoryLevelUnit;
		var category_id = $scope.dashboardObjAudit.category;
	...
	$scope.getTaskHistoryList=function(task_id){																
		...
		if(!angular.isUndefined(task_id) && task_id!=0){
			ApiCallFactory.getTasksHistory(obj).success(function(res,status){
				...
				console.log('task History : ' + $scope.task_history);
...
//	End Multiple task completion code
	...
	$scope.getCheckTaskId=function(list){
		...
		if(list.selectedId==true){
			...
			$scope.dashboardObjAudit.tasks_list.push(obj);
		}else{
			var index=0;
			angular.forEach($scope.dashboardObjAudit.tasks_list, function (item) {
				if(item.ttrn_id==list.ttrn_id){ 
					console.log("Remove  ");  
					$scope.dashboardObjAudit.tasks_list.splice(index, 1);   
					//	  break;
				}
				index++;
			});
		}
		if($scope.dashboardObjAudit.tasks_list.length == $scope.obj.reportList.length){
			$scope.selectedAll=true;
		}else{
			$scope.selectedAll=false;
		}

		if($scope.dashboardObjAudit.tasks_list.length>=1) {
	...
	$scope.approveALLTask=function(){
		...
		if($scope.dashboardObjAudit.tasks_list.length>=1){
			ApiCallFactory.approveAllTask($scope.dashboardObjAudit).success(function(res,status){
				spinnerService.hide('html5spinner');
				if(status === 200 && res.responseMessage == "Success"){
					toaster.success("Success", "Task approved successfully.");
					$scope.showOnMultipleSelection=false;
	...
	$scope.uploadFile=function(formValid) {
		...
		if(!angular.isUndefined($scope.taskObjAudit.task_list)) {
			for (var i=0; i<$scope.taskObjAudit.task_list.length; i++) {
				formData.append("task_list", $scope.taskObjAudit.task_list[i]);
			}
		}
	...
	$scope.sendApproverApproveFormTask = function() {
