...
CMTApp.controller('dashboardController', ['$scope','$rootScope','$stateParams', '$state', 'ApiCallFactory','$location', 'DataFactory', 'Storage','toaster','spinnerService','$uibModal', '$filter', function ($scope,$rootScope,$stateParams, $state,ApiCallFactory, $location, DataFactory, Storage,toaster,spinnerService, $uibModal, $filter) {
				...
				if((legal_Date[2]>= curr_year  && legal_Date[2] <= next_year) && ((legal_Date[2]== curr_year && date_lag.getMonth() >= 3) || (legal_Date[2] == next_year && date_lag.getMonth() <= 3)) ){
					for(var i = 0;i<quarter.length;i++){					
						if (month[date_lag.getMonth()] === quarter[i]) {
							...
							if (data.status == 'watingforapproval') {
		...
		for(var i = 0; i< orga_list.length; i++){    
			...
			angular.forEach($scope.taskList, function (data) { 
				...
				if(orga_list[i]==data.orga_id){//Orga Id matched
					...
					if(data.status=='watingforapproval' && data.task_cat_law =="Internal"){
...
//	-- Start Graph Code --//
	...
	$scope.loadOverAllGraph = function(data){
		...
		// console.log('loadOverAllGraph complianceChartData : ' + $scope.complianceChartData.NonComplied);
		var background = {
				type: 'linearGradient',
				x0: 0,
				y0: 0,
			...
			series: [
				{
					...
					explodedRadius: 10,
					explodedSlices: [6],
					data: [['Complied', $scope.complianceChartData.Complied], ['Posing', $scope.complianceChartData.PosingRisk], ['Non-Complied', $scope.complianceChartData.NonComplied], ['Delayed', $scope.complianceChartData.Delayed], ['Delayed-Reported', $scope.complianceChartData.Delayed_Reported], ['Waiting For Approval', $scope.complianceChartData.WaitingForApproval], ['Re-Opened', $scope.complianceChartData.ReOpened]]
				}
				]
		});
			...
			series: [
				{
					...
					explodedRadius: 10,
					explodedSlices: [6],
					data: [['Complied', $scope.internalComplianceChartData.Complied], ['Posing', $scope.internalComplianceChartData.PosingRisk], ['Non-Complied', $scope.internalComplianceChartData.NonComplied], ['Delayed', $scope.internalComplianceChartData.Delayed],['Delayed-Reported', $scope.internalComplianceChartData.Delayed_Reported], ['Waiting For Approval', $scope.internalComplianceChartData.WaitingForApproval], ['Re-Opened', $scope.internalComplianceChartData.ReOpened] ]
				}
				]
		});
	...
	$scope.drilledReport = function(obj){
		...
		angular.forEach($scope.taskList, function (data) {
			...
			if(obj.chart_name=="overall"){
				...
				if( obj.status=="Waiting For Approval" && data.status=='watingforapproval'){
					reportList.push(data);
			...
			if(obj.chart_name=="categoryLevel"){
				...
				if(loca_type == data.loca_type && obj.status=="Waiting For Approval" && data.status=='watingforapproval' && (!(from && to) || task_date>=from_date && task_date <=from_to)){
					if(data.task_type=='Main')
						reportList.push(data);
					else
						reportSubTaskList.push(data);
			...
			if(obj.chart_name=="internal"){
				...
				if( obj.status=="Waiting For Approval" && data.status=='watingforapproval' && (!(from && to) || task_date>=from_date && task_date <=from_to)){
					if(data.task_cat_law=='Internal')
						reportList.push(data);
					/*else
					reportSubTaskList.push(data);*/
			...
			if(obj.chart_name=="entityLevel") {
				...
				if(obj.entity==data.orga_name && obj.status=="Waiting For Approval" && data.status=='watingforapproval'){
					reportList.push(data);
					/*if(data.task_type=='Main')
						reportList.push(data);
					else
			...
			if(obj.chart_name=="unitLevel"){
				...
				if((!orga_id || data.orga_id === orga_id) && obj.unit == data.loca_name && obj.status=="Waiting For Approval" && data.status=='watingforapproval'){
					reportList.push(data);
					/*if(data.task_type=='Main')
						reportList.push(data);
					else
			...
			if(obj.chart_name=="departmentLevel"){
				...
				if((!orga_id || data.orga_id === orga_id) && (!loca_id || data.loca_id === loca_id) && obj.department == data.dept_name && obj.status=="Waiting For Approval" && data.status=='watingforapproval' && (!(from && to) || task_date>=from_date && task_date <=from_to)){
					reportList.push(data);
					/*if(data.task_type=='Main')
						reportList.push(data);
					else
